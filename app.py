...
def processar_porcentagem_br(valor):
    ...
    if isinstance(valor, str):
        ...
        try: 
            num = float(v)
            # LÓGICA CRÍTICA PARA CORRIGIR O 0%:
            # Se o número for menor ou igual a 1.05 (ex: 0.50), assume que já é decimal (50%).
            # Se for maior que 1.05 (ex: 50), assume que é inteiro e divide por 100.
            if num > 1.05: 
                return num / 100.0
            return num
        except: 
...
# --- TRATAMENTO DE ARQUIVOS ROBUSTO (CORREÇÃO DE COLUNAS) ---
def tratar_arquivo_especial(df, nome_arquivo):
    ...
    # Prioridade para colunas que parecem resultado
    prioridades = ['% atingimento', 'atingimento', 'resultado', 'nota final', 'score']
    # Se não achar prioridade, tenta pelo nome do arquivo (ex: procura 'csat' na coluna se o arquivo for csat.csv)
...
# --- VISÃO OPERADOR ---
    ...
    with tab_res:
        ...
        # Se não achou exato, tenta contem
