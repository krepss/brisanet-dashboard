...
# --- 2. CSS PREMIUM (ALTO CONTRASTE & LEGIBILIDADE) ---
st.markdown("""
<style>
    ...
    [data-testid="stForm"], div.stMetric, .vacation-card {
    ...
    .vacation-note { font-size: 0.9em; color: #999; font-style: italic; }
...
def normalizar_chave(texto):
    ...
    # Remove espaços extras nas pontas e NO MEIO (Ex: "ANA  PAULA" vira "ANA PAULA")
...
# --- TRATAMENTO DE ARQUIVOS ROBUSTO (CORREÇÃO DE COLUNAS E NOMES) ---
def tratar_arquivo_especial(df, nome_arquivo):
    df.columns = [str(c).strip().lower() for c in df.columns]
    
    # 1. Achar coluna de Nome
    col_agente = None
    ...
    # APLICA A NORMALIZAÇÃO NO NOME DO ARQUIVO DE INDICADOR TAMBÉM
    df['Colaborador'] = df['Colaborador'].apply(normalizar_chave)
    
    # Caso especial: Aderencia e Conformidade juntos
    ...
    if col_ad and col_conf:
        ...
        return pd.concat(lista_retorno), f"Combinado (Ad:{col_ad}, Conf:{col_conf})"
    
    # 2. Achar coluna de Valor (Lógica Melhorada)
    col_valor = None
    ...
    else: return None, f"Coluna de Valor não encontrada (Tentamos: {prioridades + secundarios})"
    ...
    return df[cols_to_keep], f"Sucesso (Coluna usada: {col_valor})"
        ...
        try:
            ...
            if df_bruto is not None:
                ...
                if df_tratado is not None:
                    # Adicionar uma linha de exemplo no log para ver o valor
                    if not df_tratado.empty:
                        exemplo_valor = df_tratado['% Atingimento'].iloc[0]
                        log_debug[-1]['Exemplo Valor'] = f"{exemplo_valor:.4f}"
                    lista_final.append(df_tratado)
            else:
                log_debug.append({"Arquivo": arquivo, "Status": "Erro", "Detalhe": "Não conseguiu ler CSV"})
        except Exception as e:
            log_debug.append({"Arquivo": arquivo, "Status": "Erro Crítico", "Detalhe": str(e)})
    ...
    if arquivos:
        ...
        if df is not None:
            ...
            if col_email and col_nome:
                ...
                # APLICA A NORMALIZAÇÃO NO NOME DO ARQUIVO DE USUÁRIOS
...
def filtrar_por_usuarios_cadastrados(df_dados, df_users):
    ...
    # Como já normalizamos tudo com normalizar_chave, a comparação direta deve funcionar
    # Mas por segurança, garantimos o upper/strip aqui também
    df_filtrado = df_dados.copy()
    df_filtrado = df_filtrado[df_filtrado['Colaborador'].isin(lista_vip)]
...
if perfil == 'admin':
    ...
    with tabs[5]: # Tabela Geral (CORRIGIDA PARA NÃO MOSTRAR PRETO/VAZIO)
        if df_dados is not None and not df_dados.empty:
...
# --- VISÃO OPERADOR ---
    ...
    with tab_res:
        ...
        # Se não achou exato, tenta contem (pode acontecer se o usuario.csv tiver nome completo e o dado só primeiro nome)
