...
# --- DICIONÁRIO DE DICAS (SMART COACH) ---
# O sistema usa isso para dar dicas baseadas no indicador que estiver ruim
DICAS_KPI = {
    "ADERENCIA": "Fique atento aos horários de entrada, saída e pausas. Evite estourar os tempos previstos.",
    "CONFORMIDADE": "Revise os procedimentos padrão. Garanta que todas as etapas obrigatórias do atendimento sejam seguidas.",
    "INTERACOES": "Busque ser mais proativo durante o atendimento. Evite silêncio excessivo.",
    "PONTUALIDADE": "Cuidado com atrasos na primeira conexão do dia. Organize-se para logar no horário exato.",
    "CSAT": "Aposte na empatia e na escuta ativa. Garanta que o cliente desligue sentindo que você fez o possível.",
    "IR": "Foque na resolução no primeiro contato. Investigue a raiz do problema antes de propor solução.",
    "TPC": "Seja objetivo sem ser rude. Use os scripts de apoio para agilizar a coleta de informações.",
    "TAM": "Tente controlar o tempo médio, mas priorize a resolução. Evite conversas paralelas desnecessárias."
}
...
<style>
    ...
    [data-testid="stForm"], div.stMetric, .vacation-card, .feedback-card {
    ...
    .insight-title { font-weight: bold; color: #d35400; display: flex; align-items: center; gap: 8px; }
...
def filtrar_por_usuarios_cadastrados(df_dados, df_users):
    ...
    df_filtrado['TEMP_NOME_UPPER'] = df_filtrado['Colaborador'].astype(str).str.upper().str.strip()
...
if perfil == 'admin':
    ...
    tabs = st.tabs(["🚦 Semáforo", "🏆 Ranking Geral", "⏳ Evolução", "🔍 Indicadores", "💰 Comissões", "📋 Tabela Geral", "🏖️ Férias Equipe", "⚙️ Admin", "📘 Ajuda"])
    ...
    with tabs[0]: 
        if df_dados is not None:
            ...
            # --- NOVO: GERADOR DE FEEDBACK (1:1) NO SEMÁFORO ---
            ...
            if colab_feedback:
                ...
                if not user_kpis.empty:
                    ...
                    # Dica Automática
                    dica_geral = DICAS_KPI.get(pior['Indicador'], "Verifique os processos operacionais e busque ajuda da monitoria.")
    ...
    with tabs[7]: # Admin
        ...
        st.info("💡 **Dica de Ouro:** Se algum indicador estiver zerado, verifique a aba 'Diagnóstico' abaixo para ver qual coluna o sistema está lendo.")
        
        st1, st2 = st.tabs(["📤 Upload", "🔍 Diagnóstico"])
        with st1:
            ...
            if up_k and st.button("Salvar Tudo"):
                ...
                df_debug, log = carregar_dados_completo_debug() # Processa para debug
        ...
        with st2:
            st.markdown("#### 🕵️ Raio-X dos Arquivos")
...
# --- VISÃO OPERADOR ---
    ...
    # --- NOVO: SMART COACH (INSIGHT AUTOMÁTICO) ---
    ...
    if not meus_dados.empty:
        # Pega o pior indicador
        pior_kpi_row = meus_dados.sort_values(by='% Atingimento', ascending=True).iloc[0]
        nome_kpi = pior_kpi_row['Indicador']
        valor_kpi = pior_kpi_row['% Atingimento']
        
        # Só mostra dica se o indicador for abaixo de 90% (senão não precisa de coach)
        if valor_kpi < 0.90:
            dica_texto = DICAS_KPI.get(nome_kpi, "Converse com seu gestor para alinhar estratégias.")
            st.markdown(f"""
            <div class="insight-box">
                <div class="insight-title">💡 Dica do Smart Coach: <b>{formatar_nome_visual(nome_kpi)}</b></div>
                <div class="insight-text">{dica_texto} (Resultado atual: {valor_kpi:.1%})</div>
            </div>
            """, unsafe_allow_html=True)
    ...
    with tab_res:
        if not meus_dados.empty:
            ...
            for i, (_, row) in enumerate(meus_dados.iterrows()):
                ...
                else: delta_msg = "🔻 Abaixo"
                with cols[i]:
                    st.metric(formatar_nome_visual(row['Indicador']), f"{val:.2%}", delta_msg)
        else:
